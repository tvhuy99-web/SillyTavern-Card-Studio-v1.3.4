(function chatSendRecoveryCompatibilityLayer() {
  ...
  function errorText(error) { return !error ? '' : textOf(error && typeof error === 'object' && 'message' in error ? error.message : error); }
  ...
  function bodyLooksLikeChat(body) {
    ...
    if (!raw) return false;
    if (/"messages"\s*:\s*\[|"prompt"\s*:|"contents"\s*:\s*\[|"chat_history"\s*:|"conversation"\s*:|"stream"\s*:\s*true/i.test(raw)) return true;
    try {
      const parsed = JSON.parse(raw);
      if (!parsed || typeof parsed !== 'object') return false;
      return !!((Array.isArray(parsed.messages) && parsed.messages.length) || (Array.isArray(parsed.contents) && parsed.contents.length) || (Array.isArray(parsed.chat_history) && parsed.chat_history.length) || (typeof parsed.prompt === 'string' && parsed.prompt) || (parsed.stream === true && (parsed.model || parsed.messages || parsed.contents || parsed.prompt)));
  ...
  function attrText(node) {
    ...
    return [node.getAttribute('aria-label'), node.getAttribute('title'), node.getAttribute('data-action'), node.getAttribute('name'), node.textContent].map(textOf).filter(Boolean).join(' ');
  }
  ...
  function composerRoot(input) {
    ...
    for (let depth = 0; node && depth < 5; depth += 1, node = node.parentElement) if (typeof node.querySelector === 'function' && node.querySelector('button')) return node;
  ...
  function isBusyElement(node) {
    ...
    return !!(typeof node.querySelector === 'function' && node.querySelector('.animate-spin, [class*="spinner"], [class*="loading"], [aria-busy="true"], [data-loading="true"]'));
  }
  function rootIsBusy(root, input) {
    ...
    const sendButton = buttons.find((button) => SEND_TEXT_RE.test(attrText(button)) || STOP_TEXT_RE.test(attrText(button)));
  ...
  function findComposer() {
    ...
    for (const input of candidates) {
      ...
      const buttons = Array.from(root.querySelectorAll('button'));
      if (!buttons.some((button) => SEND_TEXT_RE.test(attrText(button)) || STOP_TEXT_RE.test(attrText(button)) || isBusyElement(button))) continue;
  ...
  function domShowsGenerationActive() {
  ...
  function hasActiveGeneration() { return activeConversationRequests > 0 || domShowsGenerationActive(); }
  function forceUnlock(composer) {
    if (!composer || hasActiveGeneration()) return false;
  ...
  function dispatchRecoveryDeferred(reason) {
    try { window.dispatchEvent(new CustomEvent('sts:chat-recovery-deferred', { detail: { reason: textOf(reason) || 'unknown', activeRequests: activeConversationRequests } })); } catch (_) {}
  }
  function recover(reason) {
    if (isAbortLike(reason)) return false;
    if (hasActiveGeneration()) { dispatchRecoveryDeferred(reason); return false; }
    const stamp = now();
  ...
  function scheduleRecovery(reason) { if (!isAbortLike(reason)) setTimeout(() => recover(reason), 0); }
  function finishTrackedRequest() { activeConversationRequests = Math.max(0, activeConversationRequests - 1); }
  function patchFetch() {
    if (!originalFetch) return;
    window.fetch = function patchedFetch(input, init) {
      const chatLike = isConversationRequest(input, init);
      if (chatLike) activeConversationRequests += 1;
      let promise;
      try { promise = originalFetch(input, init); }
      catch (error) {
        if (chatLike) finishTrackedRequest();
  ...
  function patchXHR() {
    ...
    NativeXHR.prototype.open = function patchedOpen(method, url) { this.__stsMethod = method; this.__stsUrl = url; return originalOpen.apply(this, arguments); };
  ...
  function watchdogTick() {
    ...
    if (hasActiveGeneration()) { busySince = 0; return; }
  ...
  window.__STS_CHAT_RECOVERY__ = Object.freeze({
    ...
    watchdogTick,
    hasActiveGeneration,
    activeConversationRequestCount: () => activeConversationRequests,
    isAbortLike
  });
})();
