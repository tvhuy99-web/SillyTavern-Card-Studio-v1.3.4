(function chatSendRecoveryCompatibilityLayer() {
  ...
  function bodyLooksLikeChat(body) {
    ...
    if (/"messages"\s*:\s*\[|"prompt"\s*:|"chat_history"\s*:|"conversation"\s*:|"stream"\s*:\s*true/i.test(raw)) {
      return true;
    }
    ...
    try {
      ...
      // Non-JSON bodies are covered by the lightweight textual checks above.
    }
    return false;
  }
  ...
  function stripBlockingState(node) {
    ...
    if (typeof node.removeAttribute === 'function') {
      ['disabled', 'readonly', 'inert', 'aria-busy', 'data-loading'].forEach((name) => node.removeAttribute(name));
    }
    if (node.style) {
      try { node.style.pointerEvents = ''; } catch (_) {}
    }
    if (node.classList) {
  ...
  function forceUnlock(composer) {
    ...
    if (typeof composer.input.focus === 'function') {
      try { composer.input.focus({ preventScroll: true }); } catch (_) {
        try { composer.input.focus(); } catch (_) {}
      }
    }
    return true;
  }
  ...
  function recover(reason) {
    ...
    try {
      window.dispatchEvent(new CustomEvent('sts:chat-recovered', { detail: { reason: textOf(reason) || 'unknown' } }));
    } catch (_) {}
    return true;
  }
  ...
  function patchFetch() {
    ...
    window.fetch = function patchedFetch(input, init) {
      ...
      try {
        ...
        if (chatLike) scheduleRecovery(error && error.message || 'fetch-throw');
        throw error;
      }
      ...
      return promise.then((response) => {
        ...
        scheduleRecovery(error && error.message || 'fetch-reject');
        throw error;
      });
    };
  }
    ...
    NativeXHR.prototype.send = function patchedSend(body) {
      ...
      if (chatLike && typeof this.addEventListener === 'function') {
        ...
        this.addEventListener('load', () => {
          if (this.status >= 400) scheduleRecovery('xhr-http-' + this.status);
        }, { once: true });
      }
  ...
  function mutationHasError(mutations) {
    for (const mutation of mutations || []) {
      for (const node of Array.from(mutation.addedNodes || [])) {
        ...
        if (text && text.length < 1200 && ERROR_TEXT_RE.test(text)) return true;
      }
    }
    return false;
  }
  ...
  function startObserver() {
    ...
    observer = new MutationObserver((mutations) => {
      if (mutationHasError(mutations)) scheduleRecovery('visible-error');
    });
  ...
  window.addEventListener('unhandledrejection', (event) => {
    const message = textOf(event && event.reason && (event.reason.message || event.reason));
    if (ERROR_TEXT_RE.test(message)) scheduleRecovery('unhandled-' + message.slice(0, 120));
  });
