(function arenaRuntimeUxGuard() {
  ...
  function suppressAccidentalImmediateStop(event, button) {
    if (!button || button.id !== 'send_but') return false;
    const now = Date.now();
    const isStop = STOP_TEXT_RE.test(attrText(button));
    if (!isStop) {
      lastSendActivationAt = now;
      return false;
    }
    if (!lastSendActivationAt || now - lastSendActivationAt > STOP_ARM_DELAY_MS) return false;
    try { event.preventDefault(); } catch (_) {}
    try { event.stopPropagation(); } catch (_) {}
    try { event.stopImmediatePropagation(); } catch (_) {}
    return true;
