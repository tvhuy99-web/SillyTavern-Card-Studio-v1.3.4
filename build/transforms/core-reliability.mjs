...
export const CORE_RELIABILITY_REPLACEMENTS = Object.freeze([
  ...
  },
  {
    ...
    newText: 'case"raw.import.regex":{if(!o.card)throw Error("No character is loaded.");let e=As(await Vh(i.content)),t=Array.isArray(e)?e:Array.isArray(e?.regex_scripts)?e.regex_scripts:Array.isArray(e?.extensions?.regex_scripts)?e.extensions.regex_scripts:e&&"object"==typeof e?[e]:[];if(!t.length)throw Error("Regex import did not contain any scripts.");t=xh(t.slice(0,1e3)).map((e,t)=>({...e,id:String(e.id||`card-regex-${Date.now()}-${t}-${Math.random().toString(36).slice(2,8)}`),scriptName:String(e.scriptName||e.script_name||$h(i.name,"Regex"))}));let r=Array.isArray(o.card.extensions?.regex_scripts)?xh(o.card.extensions.regex_scripts):[],a=r.concat(t).slice(0,1e3),s={...o.card.extensions||{},regex_scripts:a},l={...o.card,extensions:s},c={...o.extensionSettings,__cardRuntimeCardExtensions:s};return o.setSessionData({card:l,extensionSettings:c}),await(n?.({extensionSettings:c})),{ok:!0,regexes:a,imported:t.length}}case"chat.reload":'
  },
  {
