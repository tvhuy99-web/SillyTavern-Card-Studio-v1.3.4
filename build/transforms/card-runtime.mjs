...
export function applyCardRuntimeTransform(source) {
  ...
  const recoveredVariableScopes = 'Ap=(e,t,n,r,a)=>{let i=e&&Object.keys(e).length?e:__stsSeedInitialVariablesFromCard({},a,String(a?.first_mes||""),e=>jt.default.parse(e)).variables,o=Math.max(0,Math.min(t.length-1,n.messageId||0)),l=t[o]?.contextState||Tp({type:"message",message_id:o},n),c=l&&Object.keys(l).length?l:e&&Object.keys(e).length?l:i,s={chat:i,global:Tp({type:"global"},n),preset:Tp({type:"preset"},n),character:Tp({type:"character"},n),[\`message:\${o}\`]:c,"script:default":Tp({type:"script",script_id:"default"},n),"extension:third-party/JS-Slash-Runner":Tp({type:"extension",extension_id:"third-party/JS-Slash-Runner"},n)};return t.forEach((e,t)=>{let n=e.contextState;n&&Object.keys(n).length?s[\`message:\${t}\`]=n:\`message:\${t}\`in s||(s[\`message:\${t}\`]={})}),r.forEach(e=>{let t=e.value?.id||"default";s[\`script:\${t}\`]=Tp({type:"script",script_id:t},{...n,scriptId:t})}),s}';
