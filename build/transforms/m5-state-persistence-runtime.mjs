...
export function applyM5StateTransform(source) {
  ...
  code = replaceRange(code, 'let arenaStateHealed=', 't({sessionId:e',
    'let __stsVariablesHealed=!1;if(!n.variables||0===Object.keys(n.variables).length){let __stsSeed=__stsSeedSessionInitialVariablesFromCard({},l,String(n.chatHistory?.[0]?.originalRawContent||n.chatHistory?.[0]?.content||l?.first_mes||""),e=>jt.default.parse(e));if(Object.keys(__stsSeed.variables).length){n.variables=__stsSeed.variables,Array.isArray(n.chatHistory)&&n.chatHistory.length&&(!n.chatHistory[0].contextState||0===Object.keys(n.chatHistory[0].contextState).length)&&(n.chatHistory[0]={...n.chatHistory[0],contextState:JSON.parse(JSON.stringify(n.variables))}),__stsVariablesHealed=!0}}let arenaStateHealed=__stsLoaded.needsRewrite||__stsVariablesHealed;', "session old arena heal", true);
  code = replaceOnce(code,
